...
class IndexMetadata {
 ...
 public:
  ...
  inline const std::vector<uint32_t> &GetKeyMapping() const { 
    ...
    //LOG(INFO)<<key_map_.size()<<std::endl;
    return key_map_; 
  }
...
class IndexInfo {
 ...
 //   LOG(INFO)<<"abc"<<std::endl;
    key_schema_ = Schema::ShallowCopySchema(schema,keymap,heap_);
 //   LOG(INFO)<<"abc"<<std::endl;
 ...
 private:
  ...
  Index *CreateIndex(BufferPoolManager *buffer_pool_manager) {
    if(key_schema_->GetMaxLen()<16){
      return new BPlusTreeIndex<GenericKey<16>,RowId,GenericComparator<16>>(meta_data_->index_id_,key_schema_,buffer_pool_manager);
    }
    else if(key_schema_->GetMaxLen()<32){
      return new BPlusTreeIndex<GenericKey<32>,RowId,GenericComparator<32>>(meta_data_->index_id_,key_schema_,buffer_pool_manager);
    }
    else{
      return new BPlusTreeIndex<GenericKey<32>,RowId,GenericComparator<32>>(meta_data_->index_id_,key_schema_,buffer_pool_manager);
    }
